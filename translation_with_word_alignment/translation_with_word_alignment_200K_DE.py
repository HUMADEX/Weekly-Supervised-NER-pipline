...
tokenizer_hl_en_de = AutoTokenizer.from_pretrained("Helsinki-NLP/opus-mt-en-de")
model_hl_en_de = AutoModelForSeq2SeqLM.from_pretrained("Helsinki-NLP/opus-mt-en-de")
...
def translation_model (sample_sentence):
    ...
    input_en_de = tokenizer_hl_en_de(sample_sentence, return_tensors="pt").input_ids
    outputs_en_de = model_hl_en_de.generate(input_en_de, max_new_tokens=80, do_sample=True, top_k=30, top_p=0.95)
    translation_de = tokenizer_hl_en_de.decode(outputs_en_de[0], skip_special_tokens=True)    
...
def word_alignment (src,tgt,sample_labels):
    ...
    for i, j in sorted(align_words):
        #print('sample_labels[i] ', sample_labels[i])
        #print('i ', i)
...
def translation(sample, sample_labels):
    #max_length = 4999
    max_sequence_length = 510  # Maximum sequence length for the model
...
# Load the dataset
df_original_dataset = pd.read_parquet("/Dockers/Datasets/libraries/E2E_Pipeline/SMILE_project/Dataset_Translations/main_dataset_english_stanza.parquet")
df_original_dataset = df_original_dataset[110000:len(df_original_dataset)]
...
def process_chunks(df, chunk_size=10000):
    ...
    for start in range(0, num_rows, chunk_size):
        ...
        chunk_filename = f'subset_2OOK_main_dataset_translated_DE_10K_chunk_after_110K_{start // chunk_size}.csv'
